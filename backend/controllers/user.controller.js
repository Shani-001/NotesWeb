...
try { 
  const user=await User.findById(userId).select("firstName lastName Email Password");
   if (!user) {
...
export const updateUserSetting=async (req,res)=>{
 const {userId}=req;
 if(!userId){
res.status(404).json({ success: false, message: "User is not authenticted" })
}
    try {
      const { firstName,lastName, email,password } = req.body;
       const hashedPassword=await bcrypt.hash(password, 10);
      const user = await User.findByIdAndUpdate(
       ...
       res.status(200).json({ success: true, message: "Settings updated", settings: user });
    } catch (error) {
...
export const purchases=async (req,res)=>{
    ...
    try {
        ...
        if(!purchased){
            return res.status(500).json({message:"No detail found of purchased notes"})
        }
        ...
        res.status(201).json({message:"Notes Extracted Successfully ",purchased,notesData})
    } catch (error) {
